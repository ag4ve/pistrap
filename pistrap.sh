...
rm -f /debconf.set
apt-get -qq update
apt-get -qq -y install --no-install-recommends git-core binutils ca-certificates locales console-common ntp ntpdate fake-hwclock openssh-server wget module-init-tools avahi-daemon cpufrequtils sysfsutils haveged rng-tools curl $choices
